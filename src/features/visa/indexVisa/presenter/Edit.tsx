import { cancelPartyCode, createIndexVisa, createPartyCode, readIndexVisa, readJobOrderQuantity, updateIndexVisa } from "../repository";
import { useEffect, useState } from "react";
import { FullScreenModal } from "../../../../componenets/Modal";
import { DateInput, UnlabeledInput } from "../../../../componenets/Input";
import { SectorInterface } from "../../../masters/sector/type";
import { CompanyInterface } from "../../../masters/company/type";
import { IndexVisaInterface, JobOrderQuantity, VisaAllocationAdapter, VisaAllocationConverter, VisaAllocationInterface, selectConverForBlockVisa } from "../type";
...
import { VisaProfessionInterface } from "../../indexFull/type/VisaProfession";


export default function Main(props: {
    onClose: any, fetchIndexVisaList: any,
    ...
    const [visaAllocationList, setVisaAllocationList] = useState<VisaAllocationInterface[]>([])
    ...
    async function onClickAdd() {
        ...
        const flag = await updateIndexVisa(parseInt(indexVisa.party_code ?? "0"), newArray)
    ...
    const fetchIndexVisa = async () => {
        const data = await readIndexVisa(props.currentElement.id ?? 0);
        if (data) {
            setIndexVisa(data);
            setVisaProfessionList(data.visaProfessionList ?? [])
            setVisaAllocationList(data.visaAllocationList ?? [])
        }
    }
    ...
    useEffect(() => {
        if (indexVisa.party_code == '' && indexVisa.jobOrderNo != "")
            getPartCode()
            ...
            <div className=" grid grid-cols-1 py-3  gap-2 shadow">
                ...
                <UpdateContentBox>
                    ...
                    <CustomSelectComponentUnlabeled
                        ...
                        options={selectOptionConveter({ options: JobOrderList, options_struct: { name: "jobOrderNumber", value: "jobOrderNumber" } })}
                ...
                {/* from block visa */}
                {showDataFromBlockVisa ? <> <UpdateContentBox>
                    <SubHeading1 text=" Block Visa :" />
                    <CustomSelectComponentUnlabeled
                        onChange={(value) => setSelectedBlockVisa(value)}
                        options={selectConverForBlockVisa(blockVisaList ?? [], props.companyList)}
                        value={selectedBlockVisa}
                    />
                </UpdateContentBox>
                    <UpdateContentBox>
                        <SubHeading1 text=" Balance Quantity :" /> {blockVisa?.balanceQuantity}
