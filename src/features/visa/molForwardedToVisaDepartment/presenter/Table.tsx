...
const Table = (props: {
  ...
  function onUpdateRow(
    index: number,
    rowData: MolForwardedTovisaDepartmentDataInterface
  ) {
    const nextData: MolForwardedTovisaDepartmentDataInterface[] =
      props.jobOrderList.map((e, i) => {
        if (i === index) {
                ...
                <CustomSingleCheckBox
                  onChange={(value) => {
                    ...
                    if (value)
                      onUpdateRow(index, {
                        ...ele,
                        molForwarded: value ? 1 : 0,
                        molForwardedDate: `${year}-${month}-${day}`,
                      });
                    else {
                      onUpdateRow(index, {
                        ...ele,
