...
const CandidateTable = (props: {
    ...
    function onUpdateRow(index: number, rowData: Submission_Dash_CandidateInterface) {
        const nextData: Submission_Dash_CandidateInterface[] = props.candidateList.map((e, i) => {
            if (i === index) {
...
const TableData = (
    props: {
        ...
        onUpdate: (index: number, rowData: Submission_Dash_CandidateInterface) => void;
        onChange: string
    }

) => {

    const [localRowData, setLocalRowData] = useState<Submission_Dash_CandidateInterface>(initValue)
    useEffect(() => {
    ...
    return (
        <TableRow3 key={props.index}>
            ...
            <TableCell3 >
                <DateInput
                    onChange={(value) => setLocalRowData({ ...localRowData, visaAppliedDate: value })}
