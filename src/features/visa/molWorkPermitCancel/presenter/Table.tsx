...
const Table = (props: {
  snoBase: number;
  MolWorkPermitList: MolForwardedTovisaDepartmentDataInterface[];
        ...
        <TableBody3>
          {props.MolWorkPermitList.map((ele: any, index: any) => (
            <TableRow3 key={index}>
              ...
              <TableCell3>
                {" "}
                {convertDateFormat(ele.submittedToCompanyDate)}
              </TableCell3>
              <TableCell3>
                {authPermissionList.url_has("delete") ? (
                  <RedButton
                    text="Cancel"
                    onClick={() => {
                      props.setModalName("cancel"), props.setCurrentData(ele);
                    }}
                  />
                ) : (
                  ""
