          ...
          <TableBody3>
            {props.ServiceChargesList?.map((ele: any, index: any) => (
              <TableRow3 key={index + 1}>
                ...
                <TableCell3>  <UnlabeledInput type="text" value={ele.service_charges}  onchange={(value)=> onUpdateRow(index,{...ele ,service_charges:value })}/></TableCell3>
                <TableCell3>  <UnlabeledInput type="text" value={ele.agent_commission}  onchange={(value)=> onUpdateRow(index,{...ele ,agent_commission:value })}/></TableCell3>
                <TableCell3> {ele.waive_off_sector_charges}</TableCell3>
                <TableCell3>  <UnlabeledInput type="text" value={ele.comments}  onchange={(value)=> onUpdateRow(index,{...ele ,comments:value })}/></TableCell3>
                <TableCell3> 
                <Checkbox value={"PROVIDED BY CO."}  checked={selectedCheckbox[index]?.isChecked === `${ele.id}PROVIDED`} onChange={()=> handleCheckboxChange(`${ele.id}PROVIDED`,index)}/>PROVIDED BY CO.
                <Checkbox value={"BY AGENCY"} checked={selectedCheckbox[index]?.isChecked === `${ele.id}AGENCY`} onChange={()=> handleCheckboxChange(`${ele.id}AGENCY`,index)}/> BY AGENCY
                <Checkbox value={"RAISE INV"} checked={selectedCheckbox[index]?.isChecked === `${ele.id}RAISE`} onChange={()=> handleCheckboxChange(`${ele.id}RAISE`,index)}/>RAISE INV
                
                
                </TableCell3>
                <TableCell3> <Checkbox /></TableCell3>
                <TableCell3>   <UnlabeledInput type="text" value={ele.invoice_service_charges}  onchange={(value)=> onUpdateRow(index,{...ele ,invoice_service_charges:value })}/></TableCell3>
                <TableCell3> 
