...
export default function Main(props: {
  ...
  async function onClickAdd() {
    ...
    for (let i = 0; i < bulkPaymentList.length; i++) {
      if (amount_to_be_deduct < 1) continue;
  ...
  const [bulkPaymentList, setBulkPaymentList] = useState<
    ...
    >
      ...
      <UpdateContentBox>
        ...
        <UnlabeledInput
          type="number"
          value={totalAmount}
          onchange={(value) => setTotalAmount(parseInt(value))}
        />
      </UpdateContentBox>
      <UpdateContentBox>
        <SubHeading1 text="Description :" />
        <TextAreaInput id="Dfs" onChange={(value) => setDescription(value)} />
      </UpdateContentBox>
