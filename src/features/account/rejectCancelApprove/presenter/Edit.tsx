import { createRejectCancelApprove, readRejectCancelApprove, updateRejectCancelApprove } from "../repository";
...
export default function Main(props: {
    onClose: ()=>void, 
    fetchRejectCancelApproveList: ()=>void,
    currentElement:RejectCancelApproveInterface, 
    ...
    async function onClickAdd() {
        ...
        // call create
        const newArray :any={...RejectCancelApprove,visaProfessionList:visaProfessionList}
   
        const flag = await updateRejectCancelApprove(1,newArray)
    ...
    const fetchvisaAuhorisationList = async () => {
        const data :any = await readVisaAuthorisationList();
        if (data) {
            setvisaAuhorisationList(data);
        }
    }
    const fetchRejectCancelApprove= async () => {
        const data :any= await readRejectCancelApprove(props.currentElement.id??0);
        if (data) {
            setRejectCancelApprove(data);
            setVisaProfessionList(data.visaProfessionList??[])
        }
    }
    useEffect(() => {
        fetchvisaAuhorisationList();
        fetchRejectCancelApprove()
        // setRejectCancelApprove(props.currentElement)
