...
export default function Main(props: {
    ...
    async function onClickAdd() {
        ...
        // call create
        const newArray :any={...blockVisa,visaProfessionList:visaProfessionList}
   
        // const flag = await updateBlockVisa(props.currentElement.id??0,newArray)
        const flag = await updateBlockVisa(newArray)
    ...
    const fetchBlockVisa= async () => {
        const data :any = await readBlockVisa(props.currentElement.id??0);
        if (data) {
            setBlockVisa(data);
