...
import { deleteJobOrder, readAssignToRMsMgrList } from "../repository";
...
export default function Main() {
  ...
  const fetchAsignToOpMGRList = async (page?: number) => {
    const data = await readAssignToRMsMgrList();
    console.log(data);
    setJobOrderList(data);
  ...
  useEffect(() => {
    // fetchJobOrderList()
    fetchAsignToOpMGRList(additionalData.pagination.page);
    fetchSectorList();
    fetchcomapanyList();
    fetchCountryList();
  }, []);
  ...
  return (
    <div>
      <CustomNavbarV3
        pageName="Assign Recruit Manager"
